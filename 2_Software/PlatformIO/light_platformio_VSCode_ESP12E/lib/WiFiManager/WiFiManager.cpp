...
boolean WiFiManager::autoConnect(char const *apName, char const *apPassword) {
  ...
  // if(wifiIsSaved){
    ...
    // 尝试使用保存的设置连接，如果失败则回退到 AP 配置
    if(!WiFi.enableSTA(true)){ // 在原模式上增加STA模式
    ...
    // 如果已经连接，或者尝试存储连接
    // @note @todo ESP32没有自动连接，所以 connectwifi 将始终被调用，除非用户之前调用了 begin 等
    // @todo 检查是否正确的 ssid == 已连接时保存的 ssid
    ...
    if (WiFi.status() == WL_CONNECTED){ // 如果wifi已连接
      ...
      #endif
      setSTAConfig();  // 设置STA的配置信息
      // @todo not sure if this is safe, causes dup setSTAConfig in connectwifi,
      // and we have no idea WHAT we are connected to
      // @todo 不确定这是否安全，导致 connectwifi 中的 dup setSTAConfig，我们不知道我们连接到什么
    }

    if(connected || connectWifi(_defaultssid, _defaultpass) == WL_CONNECTED){ // 如果wifi已连接，或者wifi连接成功
      //connected
      #ifdef WM_DEBUG_LEVEL
  ...
  if (!_enableConfigPortal) { // 如果未启用配置模式
    #ifdef WM_DEBUG_LEVEL
  ...
  // not connected start configportal wifi未连接，启动配置模式。
...
bool WiFiManager::startAP(){
  ...
  // setup optional soft AP static ip config 设置可选的软AP静态ip配置
  if (_ap_static_ip) {
    #ifdef WM_DEBUG_LEVEL
  ...
  int32_t channel = 0;
  if(_channelSync) channel = WiFi.channel(); //获取信道
...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  if(configPortalActive){ // 配置门户网站已激活
    #ifdef WM_DEBUG_LEVEL
  ...
  if(_apName == "") _apName = getDefaultAPName(); //获取默认AP名称：芯片名+MAC地址

  if(!validApPassword()) return false; // 检查AP密码是否有限
  ...
  // 处理STA连接问题，如果未连接则关闭STA，否则这将导致信道扫描挂起，无法响应SoftAP。
  ...
  configPortalActive = true; // 激活配置门户网站标志
