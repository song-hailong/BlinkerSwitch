...
#define BLINKER_OTA_VERSION_CODE "3.0.2" //版本号
#include <Arduino.h>
#include <Blinker.h>
#include <WiFiManager.h> // https://github.com/tzapu/WiFiManager
#include "main.h"

char auth[BLINKER_AUTHKEY_SIZE] = ""; // app中获取到的Secret Key(密钥)
char ssid[BLINKER_SSID_SIZE] = "";  // WIFI名称
char pswd[BLINKER_PSWD_SIZE] = "";    // WIFI密码

MyLightState LightState; //灯的状态
WiFiManager wm; //初始化WiFiManager
...
void setup()
{
    ...
    EEPROM.get(2448, auth);
    EEPROM.commit();
    EEPROM.end();
    Serial.println(auth);

//    wm.resetSettings(); // 清除保存的WiFi信息，这些信息为esp library进行储存

    if (strlen(ssid) == 0) {
        //    尝试使用已保存的WiFi连接凭据自动连接WiFi；
        //    如果连接失败，将自动创建一个具有指定名称的访问点（如果名称为空，则会自动生成SSID），如果密码为空，则为匿名访问点；
        //    进入阻塞循环等待配置，并返回成功结果。
        bool res;
        ...
        if (!res) { //AP配网失败
            ...
            if (wm._auth != "") {
                memcpy(auth,wm._auth.c_str(),BLINKER_AUTHKEY_SIZE);
                EEPROM.begin(BLINKER_EEP_SIZE);
                EEPROM.put(2448, auth);
                EEPROM.commit();
                EEPROM.end();
            }
            memcpy(ssid,wm.getWiFiSSID().c_str(),BLINKER_SSID_SIZE);
            memcpy(pswd,wm.getWiFiPass().c_str(),BLINKER_PSWD_SIZE);
        }
    }
...
void loop()
{
    ...
    if (LightState.SwitchNum >= LightState.SwitchNumReset){
        wm.resetSettings(); // 清除保存的WiFi信息，这些信息为esp library进行储存
        ESP.restart(); // 重启复位esp32
    }
}
