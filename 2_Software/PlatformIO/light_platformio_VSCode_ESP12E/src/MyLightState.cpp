...
//    this->WallSwitchState = digitalRead(KEY);  // 获取墙壁开关此时的状态
    ...
    SwitchNum = 0; // SwitchNumResetInTime秒钟内开关按下的次数
    Switch1NumTime = 0; // 首次按下时间
    SwitchNumReset = 5; // 按 SwitchNumReset 次重置
    SwitchNumResetInTime = 2; // 在 SwitchNumResetInTime 秒内按 SwitchNumReset 次重置
}
...
void MyLightState::WallSwitchControl()
{
    if (digitalRead(KEY) != this->WallSwitchState) //如果墙壁开关当前状态发生改变
    {
        ...
        if (digitalRead(KEY) != this->WallSwitchState)
        {
            ...
            if (SwitchNum == 0) //如果是第一次按下，记录当前时间
                Switch1NumTime = millis();
            SwitchNum++;
...
//            Serial.println(SwitchNum);
    ...
    if (SwitchNum !=0 && millis() - Switch1NumTime > SwitchNumResetInTime*1000) // SwitchNumResetInTime秒钟后按键按下次数清零
        SwitchNum = 0;
}
